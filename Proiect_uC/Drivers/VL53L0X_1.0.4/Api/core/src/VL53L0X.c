...
uint8_t g_stopVariable; // read by init and used when starting measurement
...
void writeReg16Bit(uint8_t reg, uint16_t value){
  msgBuffer[0] = (value >> 8) & 0xFF; // MSB first
...
void writeReg32Bit(uint8_t reg, uint32_t value){
  msgBuffer[0] = (value >> 24) & 0xFF; // MSB first
...
uint16_t readReg16Bit(uint8_t reg) {
  ...
  value = ((uint16_t)msgBuffer[0] << 8) | msgBuffer[1]; // MSB first
  return value;
}
...
uint32_t readReg32Bit(uint8_t reg) {
  ...
  value = ((uint32_t)msgBuffer[0] << 24) | ((uint32_t)msgBuffer[1] << 16) | 
          ((uint32_t)msgBuffer[2] << 8) | msgBuffer[3];
  return value;
}
...
// Get the VCSEL pulse period in PCLKs
